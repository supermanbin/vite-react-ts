...
export default function Input({className,inputFor, labelText, value, placeholder, type, onChange}:InputProps) {
  return (
    <div className={`${className}`}>
      <label htmlFor={inputFor} className={inputLabel}>{labelText}</label>
      <input className={`${inputCls}`} type={type} placeholder={placeholder} value={value} onChange={(v) => {onChange(v)}} />
    </div>
  )
}